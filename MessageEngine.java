...
public class MessageEngine {//メッセージウィンドウで使用する文字についてのクラス

    public static final int FONT_WIDTH = 16;//使用するフォントの幅
    public static final int FONT_HEIGHT = 22;//使用するフォントの高さ


    public static final int WHITE = 0;//白色の文字は画像のなかで座標0からとってこれる
    public static final int RED = 160;//赤色は160からとってこれる
    public static final int GREEN = 320;//緑色は320からとってこれる
    public static final int BLUE = 480;//青色は480からとってこれる

    private Image fontImage;//fontimage（要は文字の画像

    //hashmapで文字とその座標をつなげる
    //要はその文字の画像を使うときの座標をkana2posで特定できるようになるということ
    private HashMap<Character, Point> kana2Pos;

    private int color;//文字の色に関する値

    public MessageEngine() {//messageengineのメソッド

        ImageIcon icon = new ImageIcon(getClass().getResource("font.gif"));//文字の画像全体をロード
        fontImage = icon.getImage();//fontimageに画像を渡す

        color = WHITE;//文字の色は白


        kana2Pos = new HashMap<Character, Point>();//座標と画像（文字列）をつなげる役割（前述
        createHash();//createhash（内容は後述されている）
    }//messageengineのメソッドおわり

    public void setColor(int c) {//文字色についてのメソッド
        this.color = c;//色を取得
        //色が白でも赤でも緑でも青でもないのなら
        if (color != WHITE && color != RED && color != GREEN && color != BLUE) {
            this.color = WHITE;//色は白で固定
        }//if文おわり
    }//文字色についてのメソッドおわり


    public void drawMessage(int x, int y, String message, Graphics g) {//drawmessageについてのメソッド
        //messageの長さまでfor文を回す
    	for (int i=0; i<message.length(); i++) {
            char c = message.charAt(i);//何番目の文字かを把握する
            int dx = x + FONT_WIDTH * i;//文字の横幅を決定
            drawCharacter(dx, y, c, g);//文字を描く
        }//for文おわり
    }//drawmessageおわり


    public void drawCharacter(int x, int y, char c, Graphics g) {//drawcharacterで文字を書くメソッド
    	//要は文字を決める関数（x,y座標空間内の指定）
        Point pos = (Point)kana2Pos.get(new Character(c));
        //drawimageでfontimageから文字を切り取って張るような関数(場所などの指定は前述）
 ...
 void createHash() {
        ...
        kana2Pos.put(new Character('ち'), new Point(16, 286));
